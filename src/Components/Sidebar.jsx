...
const Sidebar = ({ setpopup, col, setcol }) => {
    const [user] = useAuthState(auth)
    return (
        user && <motion.div className={col ? 'side-bar dark-bar' : 'side-bar'}>
            <motion.div className='info'>
                {user.photoURL ? <motion.img src={user.photoURL} alt={Avatar} initial={{ opacity: 0, x: -40 }} transition={{ delay: .1 }} animate={{ opacity: 1, x: 0 }} /> : <motion.img src={Avatar} initial={{ opacity: 0, x: -40 }} transition={{ delay: .1 }} animate={{ opacity: 1, x: 0 }} />}
                <motion.h4 className='n' onClick={() => setpopup(true)} initial={{ opacity: 0, x: -40 }} transition={{ delay: .2 }} animate={{ opacity: 1, x: 0 }}>{user.displayName}</motion.h4>
            </motion.div>
            <motion.div className='side-div'>
                <motion.label className="switch" initial={{ opacity: 0, x: -50 }} transition={{ delay: .4 }} animate={{ opacity: 1, x: 0 }} >
                    ...
                    <input type="checkbox" className="input" onChange={({ target: { checked } }) => {
                        if (checked) setcol(true)
                        else setcol(false)

                    }} />
                    <span className="slider"></span>
                </motion.label>
                <motion.div className='butt' initial={{ opacity: 0, x: -60 }} transition={{ delay: .5 }} animate={{ opacity: 1, x: 0 }}>
                    <Logout />
                </motion.div>
            </motion.div>
        </motion.div>
